...
def page_multiplayer():
    ...
    from utils.multiplayer import (create_battle_room, join_battle_room, get_room,
                                    submit_answer, finish_battle, list_open_rooms)
    from utils.aria import aria_generate_battle_questions
    from data.universe import BATTLE_QUESTIONS
    import random

    # User Profile (gs variable assume ho raha hai globally available hai)
    username = gs.get("username", "Commander")
    avatar = gs.get("avatar", "🧑‍🚀")

    # State Initialization
    if "mp_room_id" not in st.session_state: st.session_state.mp_room_id = None
    if "mp_q_idx" not in st.session_state: st.session_state.mp_q_idx = 0
    if "mp_answered" not in st.session_state: st.session_state.mp_answered = None
    ...
    if not room_id:
        ...
        with tab_join:
            code = st.text_input("Enter Room Code:", placeholder="ABC123").upper().strip()
            if st.button("⚔️ Join Room", use_container_width=True):
        ...
        with tab_browse:
            ...
            for room in open_rooms:
                ...
                c1.write(f"🏠 {room['host']}'s Room ({room['id']}) - {room['players']}/4 players")
                if c2.button("Join", key=f"br_{room['id']}"):
                    ok, res = join_battle_room(room['id'], username, avatar)
                    if ok:
                        st.session_state.mp_room_id = res
                        st.rerun()
        return
    ...
    c1.metric("ROOM CODE", room_id)
    c2.metric("STATUS", room['status'].upper())
    if c3.button("🚪 Leave"):
        st.session_state.mp_room_id = None
        st.rerun()
    ...
    if room['status'] == "waiting":
        ...
        st.balloons()
        st.success(f"Battle Finished! Winner: {room.get('winner', 'Everyone!')}")
